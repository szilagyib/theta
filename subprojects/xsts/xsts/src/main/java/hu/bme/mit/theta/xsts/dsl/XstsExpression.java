...
final class XstsExpression {
	...
	private final CtlExprContext context;

	public XstsExpression(final DynamicScope scope, final SymbolTable typeTable, final CtlExprContext context) {
	...
	private static final class ExprCreatorVisitor extends XstsDslBaseVisitor<Expr<?>> {
		...
		@Override
		public Expr<?> visitStateFormula(final StateFormulaContext ctx) {
			if (ctx.op != null) {
				final Expr<BoolType> op = cast(ctx.op.accept(this), Bool());
				switch(ctx.oper.getType()) {
					case CTLNOT:
			...
			else if (ctx.rightOp != null) {
				final Expr<BoolType> leftOp = cast(ctx.leftOp.accept(this), Bool());
				final Expr<BoolType> rightOp = cast(ctx.rightOp.accept(this), Bool());

				switch (ctx.oper.getType()) {
					case CTLAND:
						return CtlAnd(leftOp, rightOp);
					case CTLOR:
						return CtlOr(leftOp, rightOp);
					case CTLIMPLY:
						return CtlImply(leftOp, rightOp);
					case CTLEQ:
						return CtlEq(leftOp, rightOp);
					default:
						throw new ParseException(ctx, "Unknown operator");
				}
			} else {
		...
		@Override
		public Expr<?> visitPathFormula(final PathFormulaContext ctx) {
			if (ctx.op != null) {
				final Expr<BoolType> op = cast(ctx.op.accept(this), Bool());
				switch(ctx.oper.getType()) {
					case GLOBALLY:
				...
				switch (ctx.oper.getType()) {
					case UNTIL:
						return Until(leftOp, rightOp);
					case RELEASE:
						return Release(leftOp, rightOp);
					default:
						throw new ParseException(ctx, "Unknown operator");
				}
			} else {
